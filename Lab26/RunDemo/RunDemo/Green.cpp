...
void Green::OnCollision(Object * obj)
{
    ...
    {
        Vector target = Vector(Line::Angle(Point(x, y), Point(obj->X(), obj->Y())), 5.0f * gameTime);

        if (Point::Distance(Point(x, y), Point(obj->X(), obj->Y())) < distance)
        {
        ...
        Translate(speed.XComponent() * 5.0f * gameTime, -speed.YComponent() * 5.0f * gameTime);
        Rotate(50 * gameTime);
